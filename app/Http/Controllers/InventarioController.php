...
class InventarioController extends Controller {
  ...
  public function index(Request $request) {
    // recibe número de serie y número de inventario, regresa 1 si existe
    // numero_inventario, 2 si existe numero_serie, 3 si existen los dos
    if ($request->has('numero_inventario') && $request->has('numero_serie')) {
      ...
      if ($request->has('order')) {
        ...
        if (strcmp($order,'numero_inventario') !== 0 &&
            strcmp($order,'numero_serie') !== 0 &&
            strcmp($order,'cantidad') !== 0 ) {
          $order = $order.'_id';
        }
      }
      ...
      if ($status) {
        ...
        if ($searchby == 1) { // solo por numero de serie, a Z - 0 9
          ...
          $x = Inventario::with('area', 'responsable')
            ...
            ->where(function($query) use ($txt) {
              return $query->whereHas('oficialia', function($q) use ($txt) {
                  $q->where('oficialia', 'LIKE', '%' . $txt . '%');
                })
                ->orwhereHas('responsable', function($s) use ($txt) {
                  $s->where('responsable', 'LIKE', '%' . $txt . '%');
          ...
          $a = Inventario::with('area', 'responsable')
            ...
            ->where(function($query) use ($txt) {
              return $query->where('oficialia_id', $txt)
                ->orWhere('numero_serie', 'LIKE', $txt . '%')
                ->orWhere('numero_inventario', 'LIKE', $txt . '%');
            })
            ->whereNull('fecha_baja')
  ...
  public function store(Request $request) {
    if ($request->has('data')) {
      ...
      if (isset($o->oficialia_id) && !is_null($o->oficialia_id))
        $j->oficialia_id = strlen($o->oficialia_id) == 4 ?
          "0" . $o->oficialia_id : $o->oficialia_id;
      ...
      // $ar->status = false;
      $ar->resguardo_id = null; // sacar artículo de resguardos
      $ar->folio_resguardo = null;
      $ar->area_id = 5; // cambiar área a BODEGA
