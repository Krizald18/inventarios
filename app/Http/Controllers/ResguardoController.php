...
class ResguardoController extends Controller {
  ...
  public function store(Request $request) {
    if ($request->has('data')) {
      ...
      $j->push();
      $folio = 'RE-'.strval($year).
        ($j->id < 100 ? ( $j->id < 10 ? '00'.$j->id : '0'.$j->id) : $j->id );
      $j->folio = $folio;
      $j->save();
      ...
      return Responsable::with('usuario', 'articulos_asignados')
        ->with(array('resguardos' => function($q) {
          ...
          $q->with('municipio');
        }))->orderBy('responsable', 'asc')->find($j->responsable_id);
  ...
  function generate_pdf(Request $request) {
    ...
    $folio = 'RE-'.strval($year).
      (intval($o->id) < 100 ?
        (intval($o->id) < 10 ? '00'.$o->id : '0'.$o->id) : $o->id );
    ...
    $data = [
      ...
      'oficial' => $o->oficial,
      'num_oficialia' => strlen(strval($o->num_oficialia)) < 5 ?
        '0'.strval($o->num_oficialia) : strval($o->num_oficialia),
