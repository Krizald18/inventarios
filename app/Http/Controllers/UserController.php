...
class UserController extends Controller {
	...
	public function allUsers(Request $request) {
		...
		return User::with('responsable')->with(array('responsable' => function($q) {
	...
	public function getUser(Request $request, $id) {
		// validar admin_token y user
		// recibe un id de un id, user (id) y admin_token
		$this->validate($request, [
			'user' => 'required',
			'admin_token' => 'required',
		]);
			...
			if (isset($u->perfil_id)) {
				...
				if ($admin == 1) {
					return Response::json(
						'No se puede remover al ultimo admin existente', 300
					);
				}
				$u->perfil_id = null;
				$u->admin()->delete();
				...
				$a = new Admin;
				$a->token = bcrypt(
					$request->usuario_id.'|'.round(microtime(true) * 1000));
				$u->admin()->save($a);
			}
			$u->save();
	...
	public function destroyUser(Request $request, $id) {
		// validar admin_token y user
		// recibe un id de un marca, user (id) y admin_token
		$this->validate($request, [
			'user' => 'required',
			'admin_token' => 'required',
		]);
