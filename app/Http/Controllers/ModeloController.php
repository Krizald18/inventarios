...
class ModeloController extends Controller {
  ...
  public function index(Request $request) {
    if ($request->has('articulos')) {
      return Modelo::with('marca', 'caracteristica', 'articulos', 'subgrupo')
        ->orderBy('marca_id', 'asc')
        ->orderBy('subgrupo_id', 'asc')
        ->orderBy('modelo', 'asc')
        ->get();
    } else {
      return Modelo::with('marca', 'caracteristica','subgrupo')
        ->orderBy('modelo', 'asc')
        ->get();
    }
  }
  ...
  public function store(Request $request) {
    ...
    // recibe un id de un modelo, user (id) y admin_token
    $this->validate($request, [
      ...
      'modelo' => 'required|unique_with:modelos,caracteristica_id,' .
    ...
    $m = Marca::with('subgrupos')
      ->whereHas('subgrupos', function($q) use ($request) {
        $q->where('subgrupo_id', '=', $request->subgrupo_id);
      })->where('id', $request->marca_id)->get();
    // marcas con ese id y que tengan ese subgrupo entre sus subgrupos.
