...
'InventarioCtrl', ['API', '$scope', '$interval', '$mdDialog', 'AlertService',
function(API, $scope, $interval, $mdDialog, AlertService) {
	...
	$scope.getInventario = () => {
		...
		if ($scope.inventario &&
				$scope.inventario.data.length > $scope.query.limit) {
			var temp = [];
	...
	$scope.buscaArticulo = () => {
		...
		// quitarle las letras dejo algo de cadena
		if (x > 0) {
			// la cadena resultante es menor que la inicial
			if (x < y) { 
				...
				// son sólo numeros, si es de 5 caracteres buscar por opcion 4
				// (oficialia_id o numero de inventario)
	...
	// AlertService.show(
	// 	"Función no implementada", "Esta funcón sera habilitada proximamente..."
	// );
	// $scope.printqueueprint = () => 
	// 	AlertService.show(
	// 		"Función no implementada", "Esta funcón sera habilitada proximamente..."
	// 	);
	...
	$scope.remove = () => {
		if ($scope.printqueue.length > 0 && $scope.selected2.length > 0) {
			$scope.printqueue = $scope.printqueue.filter(o =>
				$.grep($scope.selected2, x =>
					x.id == o.id).length == 0);
			$scope.selected2 = [];
		}
	};
